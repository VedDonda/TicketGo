...
const getPendingOrganizers = async (req, res) => {
  try {
    const organizers = await User.find({ role: 'ORGANIZER', isVerified: false }).select('-password').sort({ createdAt: -1 });
...
const approveOrganizer = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Organizer approved successfully' });
  } catch (error) {
...
const rejectOrganizer = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Organizer rejected and deleted' });
  } catch (error) {
