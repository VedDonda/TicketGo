/**
 * BookingPage.jsx — Dedicated booking flow page.
 * Route: /events/:id/book
 *
 * Stages: LOADING → SELECTING → CHECKOUT → SUCCESS | EXPIRED
 */
...
const STAGE = { LOADING: 'LOADING', SELECTING: 'SELECTING', CHECKOUT: 'CHECKOUT', SUCCESS: 'SUCCESS', EXPIRED: 'EXPIRED' };
...
export default function BookingPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (ok && data.success) { setEvent(data.data.event); setStage(STAGE.SELECTING); }
        else { setEventError(data.message || 'Event not found'); setStage(STAGE.SELECTING); }
      } catch {
  ...
  useEffect(() => {
    ...
    const socket = io(backendUrl, { transports: ['websocket', 'polling'], withCredentials: false });
    socketRef.current = socket;
    socket.on('connect', () => socket.emit('join:event', { eventId: event._id }));
    return () => { socket.emit('leave:event', { eventId: event._id }); socket.disconnect(); };
  }, [event?._id]);
  ...
  const cs = event ? (CATEGORY_COLORS[event.category] || CATEGORY_COLORS.OTHER) : CATEGORY_COLORS.OTHER;
  ...
  const currentStep = { [STAGE.SELECTING]: 1, [STAGE.CHECKOUT]: 2, [STAGE.SUCCESS]: 3 }[stage] || 0;
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', background: '#0a0a0d', color: '#f0f0f5' }}>
      ...
      {eventError && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
          minHeight: 'calc(100vh - 64px)', flexDirection: 'column', gap: 16, textAlign: 'center', padding: 32 }}>
      ...
      {stage === STAGE.LOADING && !eventError && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center',
          minHeight: 'calc(100vh - 64px)', color: '#8888a0' }}>
      ...
      {event && !eventError && (
        <>
          ...
          <div style={{ height: 3, background: `linear-gradient(90deg, ${cs.text}, rgba(91,95,199,0.4))` }} />
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px 80px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
              <button
                ...
                style={{
                  ...
                  color: '#8888a0', borderRadius: 8, padding: '7px 14px',
              ...
              <span style={{ color: '#8084e8', fontSize: '0.82rem', fontWeight: 600 }}>{stageLabel}</span>
            </div>
            ...
            {currentStep > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 32 }}>
                {[
                  ...
                  const color  = done ? '#22c55e' : active ? '#8084e8' : '#2a2a35';
                  return (
                    <div key={n} style={{ display: 'flex', alignItems: 'center', flex: idx < 2 ? 1 : 0 }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                        <div style={{
                          width: 32, height: 32, borderRadius: '50%',
                          background: done ? 'rgba(34,197,94,0.1)' : active ? 'rgba(128,132,232,0.1)' : '#111116',
                        ...
                        <span style={{ fontSize: '0.7rem', fontWeight: 600, color: done ? '#22c55e' : active ? '#f0f0f5' : '#55556a', whiteSpace: 'nowrap' }}>
                          {label}
                        </span>
                      </div>
                      {idx < 2 && (
                        <div style={{ flex: 1, height: 2, margin: '0 10px', marginBottom: 20, background: done ? '#22c55e' : '#1e1e28' }} />
                      )}
                    </div>
                  );
                })}
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: (stage === STAGE.SUCCESS || stage === STAGE.EXPIRED) ? '1fr' : '1fr 320px',
              ...
              <div style={{
                ...
                {/* Panel header */}
                <div style={{
                  padding: '20px 24px', borderBottom: '1px solid #1e1e28',
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                }}>
                  <div>
                    <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: '#f0f0f5' }}>
                      {stageLabel}
                    </h1>
                  ...
                  {/* Countdown — CHECKOUT only */}
                  {stage === STAGE.CHECKOUT && (
                    <div style={{
                      background: '#0d0d10', border: `1.5px solid ${timerColor}`,
                      borderRadius: 10, padding: '8px 18px', textAlign: 'center',
                    }}>
                      <div style={{ fontSize: '0.58rem', color: '#55556a', fontWeight: 700,
                        textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 1 }}>
                        Hold expires in
                      </div>
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  {stage === STAGE.CHECKOUT && holdData && (
                    <div>
                      ...
                      <div style={{
                        ...
                        <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#8084e8',
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {holdData.heldTickets.map((t) => (
                              <div key={t._id} style={{
                                ...
                                padding: '8px 12px', background: 'rgba(128,132,232,0.04)',
                                border: '1px solid #1e1e28', borderRadius: 8,
                              }}>
                                <span style={{ fontSize: '0.87rem', color: '#f0f0f5' }}>
                                  {t.section} · Row {t.row} · Seat {t.seatNumber}
                                </span>
                                <span style={{ color: '#8084e8', fontWeight: 700, fontSize: '0.87rem' }}>
                                  {formatPrice(t.price)}
                                </span>
                              </div>
                            ))}
                        ...
                        {/* Zoned — multi-zone */}
                        {holdData.type === 'ZONED_CAPACITY' && holdData.holds && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {holdData.holds.map((h) => (
                              <div key={h.holdId} style={{
                                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                padding: '8px 12px', background: 'rgba(128,132,232,0.04)',
                                border: '1px solid #1e1e28', borderRadius: 8,
                              }}>
                                <div>
                                  <div style={{ fontSize: '0.87rem', color: '#f0f0f5', fontWeight: 600 }}>
                                    {h.zoneName}
                                  </div>
                                  <div style={{ fontSize: '0.73rem', color: '#55556a' }}>
                                    {h.quantity} ticket{h.quantity > 1 ? 's' : ''} × {formatPrice(h.price)}
                                  </div>
                                </div>
                                <span style={{ color: '#8084e8', fontWeight: 700, fontSize: '0.87rem' }}>
                        ...
                        {/* Total */}
                        <div style={{
                          marginTop: 14, paddingTop: 14, borderTop: '1px solid #1e1e28',
                          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                        }}>
                          <span style={{ color: '#8888a0', fontSize: '0.85rem' }}>Total</span>
                          <span style={{ color: '#f0f0f5', fontSize: '1.4rem', fontWeight: 900 }}>
                      ...
                      {/* Demo notice */}
                      <div style={{
                        background: 'rgba(91,95,199,0.05)', border: '1px solid rgba(91,95,199,0.15)',
                        borderRadius: 10, padding: '12px 16px', marginBottom: 18,
                        fontSize: '0.8rem', color: '#8888a0', lineHeight: 1.6,
                      }}>
                        <strong style={{ color: '#8084e8' }}>Demo mode:</strong> No real payment is processed. Clicking Confirm will mark your tickets as booked instantly.
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            boxShadow: confirming ? 'none' : '0 4px 24px rgba(91,95,199,0.4)',
                          }}
                        >
                  ...
                  {stage === STAGE.SUCCESS && successData && (
                    <div style={{ textAlign: 'center', padding: '32px 16px' }}>
                      ...
                      <h2 style={{ margin: '0 0 8px', fontSize: '1.5rem', fontWeight: 900, color: '#f0f0f5' }}>
                        Booking Confirmed
                      </h2>
                      <p style={{ margin: '0 0 28px', color: '#8888a0', fontSize: '0.9rem' }}>
                        Your tickets have been booked successfully.
                      </p>

                      <div style={{
                        background: '#0d0d10', border: '1px solid #1e1e28', borderRadius: 12,
                        padding: '20px', textAlign: 'left', marginBottom: 28, maxWidth: 480, margin: '0 auto 28px',
                      }}>
                        ...
                        {successData.tickets && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {successData.tickets.map((t) => (
                              <div key={t._id} style={{
                                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                padding: '7px 10px', background: 'rgba(34,197,94,0.04)',
                                border: '1px solid rgba(34,197,94,0.12)', borderRadius: 7,
                              }}>
                                <span style={{ fontSize: '0.87rem', color: '#f0f0f5' }}>
                                  {t.section} · Row {t.row} · Seat {t.seatNumber}
                                </span>
                                <span style={{ color: '#22c55e', fontWeight: 700, fontSize: '0.78rem' }}>Booked</span>
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {successData.zones && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {successData.zones.map((z) => (
                              <div key={z.zoneName} style={{
                                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                padding: '7px 10px', background: 'rgba(34,197,94,0.04)',
                                border: '1px solid rgba(34,197,94,0.12)', borderRadius: 7,
                              }}>
                                <div>
                                  <div style={{ fontSize: '0.87rem', color: '#f0f0f5', fontWeight: 600 }}>{z.zoneName}</div>
                                  <div style={{ fontSize: '0.73rem', color: '#55556a' }}>{z.quantity} ticket{z.quantity > 1 ? 's' : ''}</div>
                                </div>
                                <span style={{ color: '#22c55e', fontWeight: 700, fontSize: '0.78rem' }}>Booked</span>
                              </div>
                            ))}
                        ...
                        <div style={{
                          ...
                          <span style={{ color: '#f0f0f5', fontSize: '1.2rem', fontWeight: 900 }}>
                  ...
                  {stage === STAGE.EXPIRED && (
                    <div style={{ textAlign: 'center', padding: '40px 16px' }}>
                      <div style={{
                        ...
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        margin: '0 auto 20px', fontSize: '1.5rem', color: '#f87171',
                      }}>!</div>
                      <h2 style={{ margin: '0 0 10px', fontSize: '1.4rem', fontWeight: 900, color: '#f0f0f5' }}>
                        Hold Expired
                      </h2>
                      <p style={{ margin: '0 0 28px', color: '#8888a0', fontSize: '0.9rem', lineHeight: 1.6 }}>
                        Your 10-minute hold has expired and the seats have been released.
                      </p>
                      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                        <button
              ...
              {/* Right — event sidebar */}
              {stage !== STAGE.SUCCESS && stage !== STAGE.EXPIRED && (
                <div style={{ position: 'sticky', top: 84, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{
                    ...
                    <div style={{ padding: 18 }}>
                      <span style={{
                        ...
                        borderRadius: 4, fontSize: '0.65rem', fontWeight: 700, padding: '2px 8px',
                        textTransform: 'uppercase', letterSpacing: '0.5px',
                      }}>
                        {event.category}
                      </span>
                      <h3 style={{ margin: '10px 0 14px', fontSize: '0.95rem', fontWeight: 800, color: '#f0f0f5', lineHeight: 1.3 }}>
                        {event.title}
                      </h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'flex', gap: 8, color: '#8888a0', fontSize: '0.82rem' }}>
                          <span style={{ color: '#5b5fc7', marginTop: 1 }}><CalIcon /></span>
                          {formatDate(event.date)} · {formatTime(event.date)}
                        </div>
                        <div style={{ display: 'flex', gap: 8, color: '#8888a0', fontSize: '0.82rem' }}>
                          <span style={{ color: '#5b5fc7', marginTop: 1 }}><PinIcon /></span>
                  ...
                  <div style={{
                    ...
                    fontSize: '0.78rem', color: '#55556a', lineHeight: 1.55,
                  }}>
                    Seats are held for 10 minutes. Complete your purchase before the timer runs out.
                  </div>
                </div>
              )}
