...
const S = {
  ...
  typeBtn: (active) => ({
    flex: 1,
    padding: "16px 12px",
    border: active ? "2px solid #5b5fc7" : "1.5px solid #2a2a35",
    borderRadius: 12,
    background: active ? "rgba(91,95,199,0.12)" : "transparent",
    color: active ? "#8084e8" : "#8888a0",
    cursor: "pointer",
    fontFamily: "Inter, sans-serif",
    fontWeight: 600,
    fontSize: "0.875rem",
  ...
  submitBtn: (loading) => ({
    width: "100%",
    padding: "16px",
    background: loading ? "#3a3d7a" : "#5b5fc7",
    border: "none",
    borderRadius: 12,
    color: "#fff",
    fontSize: "1.1rem",
  ...
  alert: (type) => ({
    ...
    marginBottom: 20,
    background:
      type === "err" ? "rgba(224,92,106,0.1)" : "rgba(78,202,139,0.1)",
    border:
      type === "err"
        ? "1px solid rgba(224,92,106,0.25)"
        : "1px solid rgba(78,202,139,0.25)",
...
export default function CreateEvent() {
  ...
  const updateSection = (i, field, val) => {
    ...
    if (errorKey && errors[errorKey]) {
      if (
        (field === "section" && val.trim()) ||
        (field === "rows" && +val >= 1) ||
        (field === "seatsPerRow" && +val >= 1) ||
        (field === "price" && +val >= 0 && val !== "")
      ) {
  ...
  const addSection = () =>
    setSeatingConfig((prev) => [...prev, EMPTY_SEAT_SECTION()]);
  const removeSection = (i) =>
    setSeatingConfig((prev) => prev.filter((_, idx) => idx !== i));

  const updateZone = (i, field, val) => {
    setZoningConfig((prev) =>
      prev.map((z, idx) => (idx === i ? { ...z, [field]: val } : z)),
    );
    const errorKey =
      field === "zoneName"
        ? `zone_${i}_name`
    ...
    if (errorKey && errors[errorKey]) {
      if (
        (field === "zoneName" && val.trim()) ||
        (field === "totalSeats" && +val >= 1) ||
        (field === "price" && +val >= 0 && val !== "")
      ) {
        setErrors((prev) => ({ ...prev, [errorKey]: null }));
      }
  ...
  const removeZone = (i) =>
    setZoningConfig((prev) => prev.filter((_, idx) => idx !== i));
  ...
  const validate = () => {
    ...
    if (eventType === "RESERVED_SEATING") {
      seatingConfig.forEach((s, i) => {
        ...
        if (!s.seatsPerRow || +s.seatsPerRow < 1)
          e[`sec_${i}_spr`] = "Seats/row ≥ 1";
      ...
      zoningConfig.forEach((z, i) => {
        ...
        if (!z.totalSeats || +z.totalSeats < 1)
          e[`zone_${i}_seats`] = "Seats ≥ 1";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!ok) {
        setAlert({
          msg: data.message || "Failed to create event",
          type: "err",
        });
        setLoading(false);
      ...
      setAlert({
        msg: "Network error. Make sure the backend server is running.",
        type: "err",
      });
      setLoading(false);
      isSubmitting.current = false;
      ...
      <div style={S.body}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ margin: 0, color: "#55556a", fontSize: "0.875rem" }}>
            Fill in the details below. Seats will be generated automatically in
            the background.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div style={S.card}>
            ...
            {fld(
              ...
              {
                type: "text",
                placeholder: "e.g. Coldplay World Tour 2025",
                value: title,
                onChange: (e) => {
            ...
            {fld(
              ...
              {
                placeholder:
                  "Describe your event — performers, highlights, rules…",
                value: description,
                onChange: (e) => {
                  const val = e.target.value;
            ...
            <div style={S.row2}>
              <div>
                <label htmlFor="ev-category" style={S.label}>
                  Category *
                </label>
                <select
                  id="ev-category"
                ...
                {fld(
                  ...
                  "input",
                  {
                    ...
                    onChange: (e) => {
                      ...
                      setDate(val);
                      if (errors.date && new Date(val) > new Date())
              ...
              {fld(
                ...
                "input",
                {
                  ...
                  onChange: (e) => {
                    ...
                    setVenueCity(val);
                    if (errors.venueCity && val.trim())
                      setErrors((prev) => ({ ...prev, venueCity: null }));
                    if (alert) setAlert(null);
                  },
                  onFocus: focusStyle,
                  onBlur: blurStyle,
              ...
              {fld(
                ...
                "input",
                {
                  ...
                  onChange: (e) => {
                    ...
                    setVenueAddress(val);
                    if (errors.venueAddress && val.trim())
                      setErrors((prev) => ({ ...prev, venueAddress: null }));
                    if (alert) setAlert(null);
                  },
                  onFocus: focusStyle,
                  onBlur: blurStyle,
              ...
              {fld(
                ...
                "input",
                {
                  ...
                  onChange: (e) => {
                    ...
                    if (errors.venueCapacity && !isNaN(val) && +val >= 1)
          ...
          <div style={S.card}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <button
                type="button"
                id="type-reserved"
                style={S.typeBtn(eventType === "RESERVED_SEATING")}
              ...
              <button
                type="button"
                id="type-zoned"
                style={S.typeBtn(eventType === "ZONED_CAPACITY")}
            ...
            {eventType === "RESERVED_SEATING" && (
              <div>
                ...
                >
                  Add one or more sections. The worker will bulk-generate all
                  seats automatically.
                </p>
                    ...
                    <div style={S.row3}>
                      <div>
                        ...
                        <input
                          ...
                          value={sec.section}
                          onChange={(e) =>
                            updateSection(i, "section", e.target.value)
                          }
                          onFocus={focusStyle}
                          onBlur={blurStyle}
                        />
                        {errors[`sec_${i}_section`] && (
                          <p style={S.err}>{errors[`sec_${i}_section`]}</p>
                        )}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={sec.rows}
                          onChange={(e) =>
                            updateSection(i, "rows", e.target.value)
                          }
                          onFocus={focusStyle}
                          onBlur={blurStyle}
                        />
                        {errors[`sec_${i}_rows`] && (
                          <p style={S.err}>{errors[`sec_${i}_rows`]}</p>
                        )}
                      </div>
                      <div>
                        ...
                        />
                        {errors[`sec_${i}_spr`] && (
                          <p style={S.err}>{errors[`sec_${i}_spr`]}</p>
                        )}
                      </div>
                    </div>
                    <div>
                      ...
                      <input
                        id={`sec-${i}-price`}
                        style={{ ...S.input, width: "33%" }}
                        type="number"
                        min={0}
                        placeholder="500"
                        value={sec.price}
                        onChange={(e) =>
                          updateSection(i, "price", e.target.value)
                        }
                        onFocus={focusStyle}
                        onBlur={blurStyle}
                      />
                      {errors[`sec_${i}_price`] && (
                        <p style={S.err}>{errors[`sec_${i}_price`]}</p>
                      )}
                    </div>
                  </div>
                ))}
                <button
                  ...
                  onClick={addSection}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = "rgba(91,95,199,0.2)";
                    e.currentTarget.style.borderStyle = "solid";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = "rgba(91,95,199,0.1)";
            ...
            {eventType === "ZONED_CAPACITY" && (
              <div>
                ...
                >
                  Add zones (e.g. Floor, Balcony, VIP). Each zone tracks its own
                  seat pool.
                </p>
                    ...
                    <div style={S.row3}>
                      <div>
                        ...
                        <input
                          id={`zone-${i}-name`}
                          style={S.input}
                          placeholder="e.g. Floor GA"
                          value={zone.zoneName}
                          onChange={(e) =>
                            updateZone(i, "zoneName", e.target.value)
                          }
                          onFocus={focusStyle}
                          onBlur={blurStyle}
                        />
                        {errors[`zone_${i}_name`] && (
                          <p style={S.err}>{errors[`zone_${i}_name`]}</p>
                        )}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          min={1}
                          placeholder="5000"
                          value={zone.totalSeats}
                          onChange={(e) =>
                            updateZone(i, "totalSeats", e.target.value)
                          }
                          onFocus={focusStyle}
                          onBlur={blurStyle}
                        />
                        {errors[`zone_${i}_seats`] && (
                          <p style={S.err}>{errors[`zone_${i}_seats`]}</p>
                        )}
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          value={zone.price}
                          onChange={(e) =>
                            updateZone(i, "price", e.target.value)
                          }
                          onFocus={focusStyle}
                          onBlur={blurStyle}
                        />
                        {errors[`zone_${i}_price`] && (
                          <p style={S.err}>{errors[`zone_${i}_price`]}</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          {(title || venueName) && (
            ...
            >
              ...
              >
                {title && (
                  <span>
                    <strong style={{ color: "#f0f0f5" }}>{title}</strong>
                  </span>
                )}
                ...
                {eventType === "RESERVED_SEATING" &&
                  seatingConfig[0]?.section && (
                    <span>
                      Sections: {seatingConfig.length} section(s) ·{" "}
                      {seatingConfig
                        .reduce(
                          (t, s) => t + (+s.rows || 0) * (+s.seatsPerRow || 0),
                          0,
                        )
                        .toLocaleString()}{" "}
                      seats total
                ...
                {eventType === "ZONED_CAPACITY" &&
                  zoningConfig[0]?.zoneName && (
                    <span>
                      Zones: {zoningConfig.length} zone(s) ·{" "}
                      {zoningConfig
                        .reduce((t, z) => t + (+z.totalSeats || 0), 0)
                        .toLocaleString()}{" "}
                      seats total
                    </span>
                  )}
          ...
          <div style={S.card}>
            ...
            <div style={{ maxWidth: 300 }}>
              <label htmlFor="ev-status" style={S.label}>
                Visibility *
              </label>
              <select
                id="ev-status"
              ...
              >
                ...
                <option value="DRAFT">
                  Draft (Hidden, you can publish later)
                </option>
              </select>
            </div>
          </div>
          <button
            ...
            style={S.submitBtn(loading || imageUploading)}
          >
            {loading
              ? "Creating Event..."
              : status === "DRAFT"
