...
const CATEGORIES = ['ALL', 'MUSIC', 'SPORTS', 'COMEDY', 'THEATRE', 'CONFERENCE', 'OTHER'];
    ...
    >
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: cs.text, borderRadius: '20px',
      ...
      <div style={{ padding: '18px', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <h3 style={{ margin: 0, fontSize: '0.97rem', fontWeight: 700, color: '#f0f0f5', lineHeight: 1.3,
          overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
          {event.title}
        </h3>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#8888a0', fontSize: '0.8rem' }}>
            <CalendarIcon />
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{
            background: 'rgba(91,95,199,0.15)', border: '1px solid rgba(91,95,199,0.35)',
            color: '#8084e8', borderRadius: '8px', fontSize: '0.78rem',
...
function SkeletonCard() {
  return (
    <div style={{ background: '#161619', border: '1px solid #2a2a35', borderRadius: '16px', overflow: 'hidden' }}>
      <div style={{ height: '140px', background: 'linear-gradient(90deg,#1e1e24 25%,#252530 50%,#1e1e24 75%)',
        backgroundSize: '400% 100%', animation: 'shimmer 1.5s infinite' }} />
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {[80, 60, 40].map(w => (
          <div key={w} style={{ height: '12px', width: `${w}%`, borderRadius: '6px',
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', background: '#0d0d0f', color: '#f0f0f5' }}>
      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 40px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            {pagination && !loading && (
              <p style={{ margin: '4px 0 0', fontSize: '0.82rem', color: '#55556a' }}>
            ...
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{
                  ...
                  border: category === cat ? '1px solid #5b5fc7' : '1px solid #2a2a35',
                  background: category === cat ? 'rgba(91,95,199,0.2)' : 'transparent',
        ...
        {search && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            <span style={{ fontSize: '0.83rem', color: '#8888a0' }}>
              Results for <strong style={{ color: '#f0f0f5' }}>"{search}"</strong>
            </span>
            <button onClick={() => { setSearch(''); setSearchInput(''); setPage(1); }} style={{
              ...
              color: '#e05c6a', borderRadius: '6px', fontSize: '0.72rem',
        ...
        {pagination && pagination.totalPages > 1 && !loading && !error && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginTop: '40px' }}>
            <button
              ...
              id="prev-page"
              style={{
                width: 36, height: 36, borderRadius: '8px', cursor: pagination.hasPrevPage ? 'pointer' : 'not-allowed',
                background: 'transparent', border: '1px solid #2a2a35', color: pagination.hasPrevPage ? '#f0f0f5' : '#2a2a35',
                display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s',
              }}
            ><ChevronIcon dir="left" /></button>

            {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)} id={`page-${p}`} style={{
                width: 36, height: 36, borderRadius: '8px', cursor: 'pointer',
                background: page === p ? '#5b5fc7' : 'transparent',
                border: page === p ? '1px solid #5b5fc7' : '1px solid #2a2a35',
            ...
            <button
              ...
              id="next-page"
              style={{
                width: 36, height: 36, borderRadius: '8px', cursor: pagination.hasNextPage ? 'pointer' : 'not-allowed',
                background: 'transparent', border: '1px solid #2a2a35', color: pagination.hasNextPage ? '#f0f0f5' : '#2a2a35',
                display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s',
              }}
            ><ChevronIcon /></button>
          </div>
      ...
      <footer style={{ padding: '32px 40px', borderTop: '1px solid #2a2a35', textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '12px' }}>
          <div style={{ width: 28, height: 28, background: '#5b5fc7', borderRadius: '8px',
