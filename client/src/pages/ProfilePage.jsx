...
export default function ProfilePage() {
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      const { ok, data } = await updateProfile({ name: editName });
      ...
      if (ok && data.success) {
        ...
        if (stored) {
          localStorage.setItem(
            "tg_user",
            JSON.stringify({ ...stored, name: data.data.name }),
          );
        }
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const { ok, data } = await resetPassword(email, fpOtp, newPwd);
      ...
      if (ok && data.success) {
        ...
        setPwdMsg({ type: "error", text: data.message || "Reset failed." });
      }
    } catch {
      setPwdMsg({ type: "error", text: "Network error. Please try again." });
    } finally {
  ...
  const inputStyle = (focused) => ({
    ...
    padding: "11px 14px",
    background: "#0d0d10",
    border: `1px solid ${focused ? "#5b5fc7" : "#2a2a35"}`,
    borderRadius: 8,
    color: "#f0f0f5",
    fontSize: "0.9rem",
      ...
      >
        ...
        >
          {[
            ...
            ...(localUser?.role === "ORGANIZER" || localUser?.role === "ADMIN"
              ? [{ id: "my-events", label: "My Events", Icon: TicketIcon }]
              : []),
            ...
            {bookingsLoading ? (
              ...
              >
                {bookings.map((booking) => {
                  const cs =
                    CATEGORY_COLORS[booking.event?.category] ||
                    CATEGORY_COLORS.OTHER;
                  const isOpen =
                    expandedEvent === booking.event?._id?.toString();
                  const totalTickets =
                    booking.tickets?.reduce(
                      (acc, t) => acc + (t.isZone ? t.quantity : 1),
                      0,
                    ) || 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        onClick={() =>
                          setExpandedEvent(
                            isOpen ? null : booking.event?._id?.toString(),
                          )
                        }
                        style={{
                          padding: "16px 20px",
                      ...
                      {isOpen && (
                        ...
                        >
                          ...
                          >
                            {(() => {
                              const isZoned = booking.tickets.some(
                                (t) => t.isZone,
                              );

                              if (isZoned) {
                                const grouped = booking.tickets.reduce(
                                  (acc, t) => {
                                    if (!acc[t.section]) {
                                      acc[t.section] = {
                                        section: t.section,
                                      ...
                                      acc[t.section].quantity +=
                                        t.quantity || 1;
                                      acc[t.section].price += t.price;
                                    }
                              ...
                              const bySection = booking.tickets.reduce(
                                (acc, t) => {
                              ...
                              return Object.entries(bySection).map(
                                ([section, seats]) => (
                                  <div
                                    key={section}
                                    style={{
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          >
                            <span
                              style={{ color: "#55556a", fontSize: "0.73rem" }}
                            >
                              <CalIcon />
                            </span>
                            <span
                              style={{ color: "#55556a", fontSize: "0.75rem" }}
                            >
              ...
              >
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ textAlign: "right", marginTop: 4 }}>
                    ...
                    >
                      {pwdSaving ? "Sending OTP..." : "Forgot password?"}
                    </button>
                  </div>
                </div>
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (
                        pwdMsg?.type === "error" &&
                        pwdMsg.text.includes("6 characters") &&
                        val.length >= 6
                      )
                        setPwdMsg(null);
                      if (
                        pwdMsg?.type === "error" &&
                        pwdMsg.text.includes("match") &&
                        val === confirmPwd
                      )
                        setPwdMsg(null);
