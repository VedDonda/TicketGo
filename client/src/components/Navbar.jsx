...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      borderBottom: '1px solid #2a2a35', position: 'sticky', top: 0, zIndex: 100,
    }}>
      <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none' }}>
        <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg,#5b5fc7,#8084e8)', borderRadius: '10px',
          display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <TicketIcon />
        </div>
        <span style={{ fontSize: '1.15rem', fontWeight: 900, color: '#f0f0f5', letterSpacing: '-0.5px' }}>TicketGo</span>
      </Link>
        ...
        {user ? (
          <>
            ...
            <Link to="/profile" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg,#5b5fc7,#8084e8)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 800, fontSize: '0.85rem', color: '#fff', boxShadow: '0 2px 10px rgba(91,95,199,0.3)' }}>{initial}</div>
              <span style={{ fontSize: '0.875rem', color: '#f0f0f5', fontWeight: 600 }}>{user.name}</span>
            </Link>
          ...
          <>
            <Link to="/login" style={{ padding: '8px 20px', border: '1.5px solid #2a2a35',
              color: '#f0f0f5', borderRadius: '10px', fontSize: '0.875rem', fontWeight: 600,
              textDecoration: 'none', transition: 'all 0.2s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = '#55556a'}
              onMouseLeave={e => e.currentTarget.style.borderColor = '#2a2a35'}
            >Sign In</Link>
            <Link to="/signup" style={{ padding: '9px 20px', background: 'linear-gradient(135deg,#5b5fc7,#8084e8)',
              color: '#fff', borderRadius: '10px', fontSize: '0.875rem', fontWeight: 700,
              textDecoration: 'none', boxShadow: '0 4px 14px rgba(91,95,199,0.3)' }}>Get Started</Link>
          </>
        )}
      </div>
    </nav>
