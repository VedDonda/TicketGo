...
const SEAT_STYLES = {
  AVAILABLE: {
    bg: '#1e3a2f', border: '#22c55e', color: '#22c55e',
    hover: '#166534', cursor: 'pointer',
  },
  HELD: {
    bg: '#3a2e1e', border: '#f59e0b', color: '#f59e0b',
    hover: '#3a2e1e', cursor: 'not-allowed',
  },
  BOOKED: {
    bg: '#1a1a22', border: '#2a2a35', color: '#55556a',
    hover: '#1a1a22', cursor: 'not-allowed',
  },
  SELECTED: {
    bg: '#2d2b5e', border: '#8084e8', color: '#c4b5fd',
    hover: '#3730a3', cursor: 'pointer',
  },
};
...
export default function SeatMap({ eventId, socket, onHoldConfirmed }) {
  const [seatData,   setSeatData]   = useState(null);   // { [section]: { [row]: [seat] } }
  const [selected,   setSelected]   = useState([]);     // [{ _id, section, row, seatNumber, price }]
  ...
  useEffect(() => {
      ...
      setSeatData((prev) => {
        ...
        for (const seat of updatedSeats) {
          if (next[seat.section]?.[seat.row]) {
            next[seat.section][seat.row] = next[seat.section][seat.row].map((s) =>
              s.seatNumber === seat.seatNumber ? { ...s, status: seat.status, isMyHold: false } : s
            );
          }
        }
        return next;
      });
      // Remove newly-BOOKED or HELD (by others) seats from our local selection
      setSelected((prev) =>
        prev.filter(
          (sel) => !updatedSeats.some(
            (u) =>
    ...
    const handleSeatReleased = () => {
      // Someone released their hold → refresh fully to get AVAILABLE status back
      loadSeats();
    };
  ...
  const toggleSeat = (seat) => {
    ...
    setSelected((prev) => {
      ...
      if (prev.length >= MAX_SEATS) {
        setHoldError(`You can select at most ${MAX_SEATS} seats per transaction.`);
        return prev;
      }
      return [...prev, seat];
    });
  ...
  const handleConfirmHold = async () => {
    ...
    try {
      ...
      if (ok && data.success) {
        ...
        // Refresh seat map — some seats may have been taken
        await loadSeats();
        setSelected([]);
      }
    } catch {
  ...
  // ── Determine seat display status (selected overrides AVAILABLE) ─────────────
  ...
  if (error) return (
    <div style={{ textAlign: 'center', padding: '40px 20px', color: '#f87171', fontFamily: 'Inter, sans-serif' }}>
      <p>{error}</p>
      <button onClick={loadSeats} style={btnStyle('#5b5fc7')}>Retry</button>
    </div>
  );
    ...
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
        {[
          ...
          return (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: '#8888a0' }}>
              <div style={{ width: 14, height: 14, borderRadius: 3, background: st.bg, border: `1px solid ${st.border}` }} />
              {label}
            </div>
          );
        })}
      ...
      {/* ── Stage indicator ──────────────────────────────────────────────── */}
      <div style={{
        textAlign: 'center', background: 'rgba(91,95,199,0.1)', border: '1px solid rgba(91,95,199,0.25)',
        borderRadius: 8, padding: '8px 20px', marginBottom: 28, color: '#8084e8',
        fontSize: '0.75rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase',
      }}>
        ▬▬▬▬▬▬▬  STAGE  ▬▬▬▬▬▬▬
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28, overflowY: 'auto', overflowX: 'auto', maxHeight: '55vh', paddingBottom: 16 }}>
        {sections.map((section) => {
          ...
          return (
            <div key={section}>
              <div style={{
                fontSize: '0.7rem', fontWeight: 800, color: '#5b5fc7',
                letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 10,
                paddingBottom: 6, borderBottom: '1px solid #1e1e28',
              }}>
                {section}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {rowKeys.map((row) => (
                  <div key={row} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {/* Row label */}
                    <span style={{
                      width: 22, fontSize: '0.7rem', color: '#55556a', fontWeight: 700,
                      ...
                      {rows[row].map((seat) => {
                        ...
                        return (
                          <button
                            ...
                            onMouseEnter={(e) => {
                              if (interactive) e.currentTarget.style.background = st.hover;
                            }}
                            onMouseLeave={(e) => {
      ...
      <div style={{
        marginTop: 24, paddingTop: 20, borderTop: '1px solid #2a2a35',
        ...
        <div>
          {totalSelected > 0 ? (
            <>
              <p style={{ margin: '0 0 3px', fontSize: '0.78rem', color: '#8888a0' }}>
                {totalSelected} seat{totalSelected !== 1 ? 's' : ''} selected
              </p>
              <p style={{ margin: 0, fontSize: '1.3rem', fontWeight: 900, color: '#f0f0f5' }}>
          ...
          {holdError && (
            <p style={{ margin: '6px 0 0', color: '#f87171', fontSize: '0.78rem' }}>{holdError}</p>
          )}
        </div>
